...
import edu.mit.mitmobile.TabConfigurator;
import edu.mit.mitmobile.TwoLineActionRow;
import edu.mit.mitmobile.objs.CourseItem;
import edu.mit.mitmobile.objs.CourseItem.CourseTime;
import edu.mit.mitmobile.people.PeopleSearchActivity;
...
public class CourseDetailsView implements SliderInterface {
	...
	void updateUI(final CourseItem courseItem) {
		...
		for (int i=0; i < courseItem.announcements.size(); i++) {
			...
			newsRow = new CoursesNewsActionRow(mActivity, null);
		...
		for (CourseTime t : courseItem.times) {
			...
			classTimeRow = new TwoLineActionRow(mActivity, null);	
		...
		if(!courseItem.description.equals("")) {
			TwoLineActionRow descriptionRow = new TwoLineActionRow(mActivity, null);
		...
		if (courseItem.staff.instructors.size() + courseItem.staff.tas.size() == 0) {
			...
			for (final String staffName : courseItem.staff.instructors) {
				TwoLineActionRow personRow = new TwoLineActionRow(mActivity, null);
